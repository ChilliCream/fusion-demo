 ...
 * hover, and a hover-revealed "Add to cart" pill. The whole tile is a
 * `react-router` `Link` to the product's detail page - the shared `Card`/
 * `Button` primitives only render plain `<a>`/`<button>` elements (no
 * `Link` support yet), so this component wraps its own tile markup in a
 * `Link` itself rather than editing those primitives. It intentionally
 * doesn't reuse the `Card` component: `Card`'s fixed `p-6` padding fights a
 * media area that has to bleed to the tile's edges, and overriding it via
 * `className` is unreliable (Tailwind utility collisions between a
 * primitive's base classes and caller overrides don't reliably resolve in
 * the caller's favor).
 *
...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Link
      to={`/products/${data.id}`}
      className="group block h-full rounded-xl no-underline focus:outline-none focus-visible:ring-2 focus-visible:ring-cc-accent"
    >
      <article className="relative flex h-full flex-col overflow-hidden rounded-xl border border-cc-card-border bg-cc-card-bg backdrop-blur-sm transition-colors group-hover:border-cc-card-border-hover">
        <div
          aria-hidden="true"
        ...
        >
          ...
          <div className="absolute right-3 bottom-3 z-10 opacity-0 transition-opacity duration-200 group-hover:opacity-100 group-focus-within:opacity-100">
            <Button
              variant="solid"
              size="sm"
